...
function create_fragment(ctx) {
	...
	return {
		c() {
			...
			div0.className = "notification-context";
			div1.className = "notification";
			dispose = listen(button, "click", ctx.removeNotifications);
		},
...
// (396:8) {#if notification.position === position}
function create_if_block(ctx) {
	...
	function select_block_type(ctx) {
		if (ctx.notificationItem) return 0;
		return 1;
	}
...
// (397:10) {#if notificationItem}
function create_if_block_1(ctx) {
	var switch_instance_anchor, current;

	var switch_value = ctx.notificationItem;

	function switch_props(ctx) {
		return { props: { notification: ctx.notification } };
	}
	...
	return {
		...
		p(changed, ctx) {
			...
			if (changed.$store) switch_instance_changes.notification = ctx.notification;

			if (switch_value !== (switch_value = ctx.notificationItem)) {
				if (switch_instance) {
					group_outros();
...
// (395:6) {#each $store as notification (notification.id)}
...
// (393:2) {#each positions as position}
function create_each_block(ctx) {
	...
	return {
		c() {
			...
			t = space();
			div.className = div_class_value = "" + (`notification-${ctx.position}`) + " svelte-g1crun";
		},
...
function create_fragment$1(ctx) {
	...
	return {
		c() {
			...
			div.className = "notifications svelte-g1crun";
		},
		...
		p(changed, ctx) {
			...
			if (changed.positions || changed.$store || changed.notificationItem) {
...
function instance$1($$self, $$props, $$invalidate) {
  ...
  let { notificationItem = null } = $$props;
  ...
  const positions = [
	...
	$$self.$set = $$props => {
		if ('notificationItem' in $$props) $$invalidate('notificationItem', notificationItem = $$props.notificationItem);
		if ('$$scope' in $$props) $$invalidate('$$scope', $$scope = $$props.$$scope);
	};
...
class Notifications extends SvelteComponent {
	constructor(options) {
		super();
		init(this, options, instance$1, create_fragment$1, safe_not_equal, ["notificationItem"]);
	}
}
